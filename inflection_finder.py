...
Won't catch irregular inflections like tener 'to have' inflected as tiene 
'(s)he has' and tengo 'i have' in Spanish, but is an organized step in 
approaching an undeciphered writing system
"""
...
class CandidateFile:
    ...
    def add_candidate_to_file(self, root, inflections):
        """
        outputs to an interim file: each line is a key-value pair 
        [ { 'root' : ['inflection', 'candidates'] } ]
        """
        ...
        self.open()
        self.shelve[root] = inflections
        self.close()


def process_inflections(inflections):
    """
    inputs a file with roots and inflection candidates, then determines 
    which inflection candidates recur and are worth investigating
    
    See also "Kober's Triplets", Alice Kober's early manual work on Linear B 
    which used a similar methodology and was instrumental in the decipherment 
    of Linear B.
    ...
    for i in inflections.keys():
        ...
        if len(inflections[i]) == 1:
            # if the inflection occurs only once, we can assume it's noise and 
            # throw it out until added data in a future run corroborates it
...
def get_clusters(raw_file):
    """
    [unimplemented] inputs a file of unique morphemes, outputs clusters 
    of potential inflections
    
    Phase I: let OpenRefine do the heavy lifting and create the file of 
    clusters (process_clusters) from raw data
